...
def queryTweetsContainingHashtag(twitter, input_list_of_target_hashtags, start_date, end_date, earliestTweet, latestTweet, maxTweets=1000000):
	...
	# if(len(input_list_of_target_hashtags)>1):
	# 	for hash_tag in input_list_of_target_hashtags[1:-1]:
	# 		query += ' OR '
	# 		query += hash_tag

	# 	query += input_list_of_target_hashtags[-1]
...
def getTweetsByHashtag(twitter, query, start_date, end_date, earliestTweet, latestTweet, max_tweets):
	...
	try :
		...
		# results = twitter.search(q=query, count=100, since_id = notBefore, result_type='recent', include_entities=True)
		tweets += results['statuses']
		if("next_results" in results["search_metadata"]):
			cursor = int(results["search_metadata"]["next_results"].split('&q')[0].split('max_id=')[-1])
		else:
			keepgoing=False;
...
# def queryAllFromTweets(twitter, input_list_of_queried_tweets):
