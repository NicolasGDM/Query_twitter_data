	...
	while (cursor!=0):
		try:
			...
			if(e.error_code==404 or e.error_code==401):
				...
				reset_in=rate_limit_status['resources']['followers'][ '/followers/ids']['reset'] - datetime.now().timestamp()
	...
	while (cursor!=0):
		try:
			...
			if(e.error_code==404 or e.error_code==401):
				...
				friends=[]
				reset_in=rate_limit_status['resources']['friends'][ '/friends/ids']['reset'] - datetime.now().timestamp()
				time.sleep(reset_in+60)
				try:
					temp_friends=twitter.get_friends_ids(user_id=user_id,cursor=cursor)
